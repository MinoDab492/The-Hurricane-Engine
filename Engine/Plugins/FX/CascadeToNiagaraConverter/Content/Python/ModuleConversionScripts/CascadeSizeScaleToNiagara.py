...
class CascadeParticleSizeScaleConverter(ModuleConverterInterface):
    ...
    def convert(cls, args):
        ...
        if emitter.find_renderer("RibbonRenderer") is not None:
            ...
            script_args = ue.CreateScriptContextArgs(ue.AssetData(Paths.script_scale_ribbon_width), [1, 0])
            ...
            script_args = ue.CreateScriptContextArgs(ue.AssetData(Paths.script_scale_mesh_size), [1, 0])
            script = emitter.find_or_add_module_script(
                "ScaleMeshSize",
                script_args,
                ue.ScriptExecutionCategory.PARTICLE_UPDATE)
            ...
            script_args = ue.CreateScriptContextArgs(ue.AssetData(Paths.script_scale_sprite_size), [1, 1])
            script = emitter.find_or_add_module_script(
                "ScaleSpriteSize",
                script_args,
                ue.ScriptExecutionCategory.PARTICLE_UPDATE)
        ...
        script_args = ue.CreateScriptContextArgs(ue.AssetData(Paths.script_particle_state), [1, 1])
