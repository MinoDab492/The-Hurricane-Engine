...
class add_null_above_selected(unreal.ToolMenuEntryScript):
    ...
    def is_visible(self, context):
        ...
        This menu entry will only display if there are only controls in the selection
        '''
        ...
        if control_rig_context:  # Confirm control rig context
            ...
            if control_rig_bp:
                ...
                for key in selected_keys:
                    ...
                    null = hierarchy_controller.add_null(name = transform_name, parent = parent, transform = transform_initial)

                    # Reparent the control to the null
                    hierarchy_controller.set_parent(key, null, maintain_global_transform=True, setup_undo=True, print_python_command=False)
                    
                    # Set initial transform
                    hierarchy.set_global_transform(null, transform_initial, initial=True)
                    hierarchy.set_global_transform(key, transform_initial, initial=True)
