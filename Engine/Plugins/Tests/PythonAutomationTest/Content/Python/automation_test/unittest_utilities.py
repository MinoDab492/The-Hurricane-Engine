...
def try_remove_file(file_path, tries=3):
    """
    Try to remove file_path X amount of time before giving up.
    """
    for n in range(tries):
        try:
            ...
            attempt_num = n + 1
            if attempt_num == tries:
                raise

            else:
                log("Failed to remove '%s' (attempt #%s)"%(file_path, attempt_num))
                time.sleep(2)
